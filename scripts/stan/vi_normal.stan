...
parameters {
  ...
  real<lower=0> sigma_a;  // error scale for intercept
}
model {
  ...
  // model intercepts
  a_state ~ normal(Q_ast_2 * theta_2 + a_region[region], sigma_a);

  // likelihood
  y ~ normal(a_state[state] + Q_ast_1 * theta_1, sigma);
}
